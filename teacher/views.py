...
def teacher_dashboard_view(request):
    ...
    total_students = Student.objects.filter(user__user_profile__school_class__teacher=teacher).count() if teacher else 0
...
def teacher_attendance_view(request):
    ...
    if selected_class_id:
        ...
        students = Student.objects.filter(user__user_profile__school_class=selected_class, user__is_active=True).order_by('user__first_name', 'user__last_name')
    ...
    if request.method == 'POST':
        ...
        class_students = Student.objects.filter(user__user_profile__school_class=cls, user__is_active=True)
